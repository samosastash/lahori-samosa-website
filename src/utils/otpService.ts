    ...
    try {
      ...
      const response = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
        ...
        body: JSON.stringify({
          ...
          template_id: 'template_5sle4gl', // We'll use the existing template for now
          user_id: 'aFnOBMy5siQAFBFJ1',
          template_params: {
            order_id: `OTP-${Date.now().toString(36).toUpperCase()}`,
            customer_name: customerName,
            customer_phone: 'Email Verification',
            customer_address: `Your verification code is: ${otp}`,
            order_items: `Please use this code to verify your email address. This code will expire in 5 minutes.`,
            total_amount: 'Email Verification Code'
          }
        })
      });
